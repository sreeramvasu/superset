...
def _common_config_kwargs(config: dict[str, Any]) -> dict[str, Any]:
    """
    Extract the connection settings shared by both Redis cache backends.

    :param config: The Superset cache configuration
    :returns: Keyword arguments common to both backends
    """
    return {
...
class RedisCacheCommandsMixin:
    """
    Redis commands shared by the async event cache backends.

    Implementers are expected to expose the underlying client as ``_cache``.
    """
