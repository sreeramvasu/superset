...
// Error entry returned by import endpoints, keyed by file name in `extra`.
interface ImportError {
  extra: Record<string, unknown> | null;
}

const getErrorEntries = (error: ImportError) =>
  Object.entries(error.extra ?? {});

interface RecentActivityData {
  other: JsonObject[];
  viewed: RecentActivity[];
}

// Internal fields of the rison module that control identifier encoding.
...
export const getRecentActivityObjs = (
  ...
  SupersetClient.get({ endpoint: recent }).then(recentsRes => {
    const res: Partial<RecentActivityData> = {};
    ...
    return getFilteredChartsandDashboards(addDangerToast, filters).then(
      ({ other }) => {
        ...
        return res as RecentActivityData;
      },
    );
  });
...
// Import error payloads carry marshmallow schema messages under `_schema`.
const getSchemaMessages = (payload: unknown): string[] => {
  const schema = (payload as { _schema?: unknown } | null | undefined)?._schema;
  return Array.isArray(schema) ? (schema as string[]) : [];
};
...
export const getPasswordsNeeded = (errors: ImportError[]) =>
  errors.flatMap(error =>
    getErrorEntries(error)
      .filter(([, payload]) => isNeedsPassword(payload))
      .map(([fileName]) => fileName),
  );

export const getSSHPasswordsNeeded = (errors: ImportError[]) =>
  errors.flatMap(error =>
    getErrorEntries(error)
...
export const getSSHPrivateKeysNeeded = (errors: ImportError[]) =>
  errors.flatMap(error =>
    getErrorEntries(error)
...
export const getSSHPrivateKeyPasswordsNeeded = (errors: ImportError[]) =>
  errors.flatMap(error =>
    getErrorEntries(error)
...
export const getAlreadyExists = (errors: ImportError[]) =>
  errors.flatMap(error =>
    getErrorEntries(error)
...
export const hasTerminalValidation = (errors: ImportError[]) =>
  errors.some(error => {
    const noIssuesCodes = getErrorEntries(error).filter(
      ([key]) => key !== 'issue_codes',
    );
